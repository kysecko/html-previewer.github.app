document.addEventListener('DOMContentLoaded', () => {
  ...
  const hideSpinner = () => {
    if (redirectSpinner) redirectSpinner.style.display = 'none';
  };
  ...
  form.addEventListener('submit', async (e) => {
    ...
    // Show spinner immediately on valid submit  
    ...
    // Show loader right away — no waiting for the API
    showSpinner();
    ...
    try {
      ...
      if (!res.ok || !data.success) {
        // Hide spinner and show error if login failed
        hideSpinner();
      ...
      // Login success: wait 700 milliseconds then redirect  
